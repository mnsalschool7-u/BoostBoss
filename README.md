...
9. Ask LlamaParse for layout and embedded image assets, then show the best extracted product photo when one is returned.
...
* Product photo display depends on LlamaParse returning image metadata for the uploaded PDF. If no standalone image is returned, the card stays empty and the original PDF preview remains available.
