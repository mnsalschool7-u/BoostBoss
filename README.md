...
9. Ask LlamaParse for embedded image assets and show an extracted product photo when one is returned.
...
* The product visual card shows an embedded image returned by LlamaParse when the PDF contains one.
...
* Product photo display depends on LlamaParse returning embedded image metadata for the uploaded PDF. If no embedded image is returned, the card stays empty and the original PDF preview remains available.
